...
trait PipelineFlowFactory {
  ...
  def create(context: Map[String, Any])(implicit system: ActorSystem):
...
class PipelineExtensionImpl(flowFactoryMap: Map[String, PipelineFlowFactory],
  ...
  def getFlow(pipelineSetting: PipelineSetting, context: Map[String, Any] = Map.empty): Option[PipelineFlow] = {
  ...
  private def buildPipeline(flowNames: Seq[String], context: Map[String, Any]) = {
